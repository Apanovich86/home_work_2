...
userDiv.innerHTML = `<span>ID: </span> ${user.id}<br>
<span>NAME:</span> ${user.name}<br>
<span>EMAIL:</span> ${user.email}<br>
<span>ADDRESS:</span><br>&nbsp&nbsp&nbsp 
<span>street:</span> ${user.address.street}<br>&nbsp&nbsp&nbsp 
<span>suite:</span>  ${user.address.suite}<br>&nbsp&nbsp&nbsp
<span>sity:</span> ${user.address.city}<br>&nbsp&nbsp&nbsp 
<span>zipcode:</span>  ${user.address.zipcode}<br>&nbsp&nbsp&nbsp 
<span>geo:</span> <br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp 
<span>lat:</span> ${user.address.geo.lat}<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp 
<span>lng:</span> ${user.address.geo.lng}<br>
<span>PHONE:</span> ${user.phone}<br>
<span>WEBSITE:</span> ${user.website}<br>
<span>COMPANY:</span><br>&nbsp&nbsp&nbsp 
<span>name:</span> ${user.company.name}<br>&nbsp&nbsp&nbsp 
<span>catchphrase:</span> ${user.company.catchPhrase}<br>&nbsp&nbsp&nbsp 
